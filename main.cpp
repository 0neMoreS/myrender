...
int main(int argc, char **argv)
{
    ...
    for (int i = 0; i < model->nfaces(); i++)
    {
        ...
        std::vector<Vec3f> screen_verts{3};
        std::vector<ScreenTriangleVert> screen_tris{3};
        ...
        for (int j = 0; j < 3; j++)
        {
            ...
            intensities[j] = normals[j] * (verts[j] - light).normalize();
            ...
            screen_verts[j] = {(float)x, (float)y, v4[2]};
            screen_tris[j] = ScreenTriangleVert{screen_verts[j], normals[j], intensities[j]};
        }
